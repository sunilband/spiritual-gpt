...
const key=process.env.OPEN_AI_API_KEY
  const openAi = new OpenAIApi(
    new Configuration({
      apiKey:"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"
      // apiKey:key
    })
  )
  console.log(key,"yoyo")
  ...
  return (
    <div className='flex flex-col justify-center z-20 md:pt-20 w-screen h-screen' >
      {/* answer field */}
    <textarea className='w-screen self-center focus:outline-none mt-16 border max-w-7xl h-72 rounded-2xl p-4 flex flex-wrap bg-[#40414f] text-white text-justify font-medium tracking-wide overflow-y-scroll no-scrollbar drop-shadow-2xl z-20'  value={answer} />
    {/* loader */}
    
    {/* input fiend */}
      <input type="text" className='self-center focus:outline-none border-2 border-[#202123] m-4 p-4 rounded-2xl bg-[#40414f] text-white w-4/5 tracking-wide z-20' value={input} onChange={setterInput} placeholder='Ask question here...'/>
...
{/* button */}
    <button className='w-44 self-center border-2 m-4 p-4 rounded-2xl bg-[#202123] text-white hover:scale-[1.02]   hover:font-bold hover:drop-shadow-2xl  transition-all ease-in-out duration-200 uppercase tracking-[7px] z-20' onClick={buttonClick}>
    Generate
    </button>
